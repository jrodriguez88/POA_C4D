...
Los conjuntos de datos se encuentran alojados en [OneDrive](https://partnersnet107.sharepoint.com/:f:/r/sites/EDHC4D/Shared%20Documents/M%26E%20C4D/z13.Equipo%20M%26E/Consultores/2021-02%20Consultor%C3%ADa%20Miner%C3%ADa%20de%20datos/2024-02%20Miner%C3%ADa%20de%20Datos%20-%20Compartida%20Consultor/2_archivos_procesados/R_data?csf=1&web=1&e=V5GSsL)
...
La Longitud de los nombres de variables Caracterizacion,  Plan de Gestion de Finca y Productividad es en promedio :`r names(caracterizacion) %>% map_dbl(nchar) %>% mean`, `r names(plan_gestion_finca) %>% map_dbl(nchar) %>% mean` y `r names(productividad) %>% map_dbl(nchar) %>% mean`, respectivamente.
...
Se recodificaron los valores de la estrategia de extension y factoees de exito, se realiza un analisis de frecuencia de visitas donde se encuentra que el promedio de dias entre visistas es de `r mean(promedio_visitas$promedio_dias_entre_visitas, na.rm = T)`.
